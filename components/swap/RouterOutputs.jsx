...
const RouterOutputs = () => {
  ...
  function defaultImage(event) {
    if (
      event.currentTarget.src.endsWith("unknown.svg") ||
      event.currentTarget.src.endsWith("unknown-white.svg")
    )
      return;
    event.currentTarget.src =
      theme === "dark" ? "/tokens/unknown-white.svg" : "/tokens/unknown.svg";
  }
        ...
        {swap.routers.map((router) => (
          <div className="router" key={router.id}>
            <div className="section">
              <img
                className="icon"
                src={`/routers/${router.routerId || router.id}.svg`}
              ></img>
            ...
            <div className="section">
              {swap.tokenIn ? (
                <img
                  className="icon"
                  src={`/tokens/${chain.id}/${swap.tokenIn.symbol}-${swap.tokenIn.address}.svg`}
              ...
              {`${
                swap.tokenIn && swap.tokenInAmount
                  ? format(parse(swap.tokenInAmount, swap.tokenIn.decimals))
                  : "..."
              } `}
              ...
              {swap.tokenOut ? (
                <img
                  className="icon"
                  src={`/tokens/${chain.id}/${swap.tokenOut.symbol}-${swap.tokenOut.address}.svg`}
                  onError={defaultImage}
                ></img>
              ) : (
                <></>
              ...
              {`${
                router.out === false ||
                !chain.swapSettings.routers[router.id].enabled
                  ? "—"
                  : swap.tokenOut && router.out
                  ? format(parse(router.out, swap.tokenOut.decimals))
                  : "..."
              } `}
            ...
            <div className="section">
              ...
              !chain.swapSettings.routers[router.id].enabled
                ? "—"
                : swap.tokenOut && router.out
                ? swap.tokenOut.default
                  ? `≈ $${formatNumber(
                      getTokenValue(swap.tokenOut, router.out)
                    )}`
                  : width > 550
                  ? "Price Unknown"
                  : "Unknown"
...
// style="-webkit-mask-image: url(&quot;https://www.layern.com/_next/image?url=https%3A%2F%2Fassets.tina.io%2F6e9301ab-737b-44a7-80b7-4c663e2c97c2%2Fsvgs%2Fwordmark.svg&w=3840&q=90&quot;);"