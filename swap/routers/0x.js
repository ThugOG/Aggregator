...
// Define your API key at the top of the file for easy access and configuration
const API_KEY = "0cb8de74-5419-40b1-b64f-b8ea8cb7851a";
...
async function quote(chain) {
  ...
  try {
    ...
    const result = await axios(
      `${endpoint}/swap/v1/price?${querystring.encode({
...
async function getSwap(chain, account) {
  ...
  try {
    ...
    const result = await axios(
      `${endpoint}/swap/v1/quote?${querystring.encode({
        sellToken: swap.tokenIn.address,
        buyToken: swap.tokenOut.address,
        sellAmount: swap.tokenInAmount.toString(),
    ...
    const gas = await chain.web3.eth
      .estimateGas({
        from: account,
        to: result.data.to,
        value:
